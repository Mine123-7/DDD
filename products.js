...
// Data storage
let cart = JSON.parse(localStorage.getItem('jazzbar_cart')) || [];
let quantities = JSON.parse(localStorage.getItem('jazzbar_quantities')) || {};

// Stats (optional)
let orderCounts = JSON.parse(localStorage.getItem('jazzBarOrderCounts')) || {};
let clickCounts = JSON.parse(localStorage.getItem('jazzBarClickCounts')) || {};
...
function loadCartFromStorage(){
  cart = JSON.parse(localStorage.getItem('jazzbar_cart')) || [];
  quantities = JSON.parse(localStorage.getItem('jazzbar_quantities')) || {};
}
...
function addToCart(productId, name, price, image) {
  const qtyEl = document.getElementById(`${productId}-qty`);
  const qty = qtyEl ? parseInt(qtyEl.textContent,10) || 0 : (quantities[productId] || 0);
  if (!qty || qty <= 0) {
    showNotification('กรุณาเลือกจำนวนสินค้าก่อน', 'warning');
    return;
  }
  ...
  if (existing) {
    ...
    cart.push({ id: productId, name, price: Number(price), image, quantity: qty });
  }
  ...
  // reset qty UI and state
  quantities[productId] = 0;
  if (qtyEl) qtyEl.textContent = '0';

  saveCart();
  updateCartDisplay();
  showNotification(`เพิ่ม ${name} ลงตะกร้า ${qty} ชิ้น`, 'success');
  animateToCart(productId, image);
}
...
function loadCountsFromStorage(){
  orderCounts = JSON.parse(localStorage.getItem('jazzBarOrderCounts')) || orderCounts || {};
  clickCounts = JSON.parse(localStorage.getItem('jazzBarClickCounts')) || clickCounts || {};
}
...
function renderOrderCountsOnProducts(){
  ...
  document.querySelectorAll('.product-card').forEach(card=>{
    ...
    if (!id) return;
    const c = orderCounts[id] || 0;
    const k = clickCounts[id] || 0;
      ...
      <div style="display:flex;gap:18px;align-items:flex-start;flex-wrap:wrap">
        ...
        <div style="flex:1;min-width:240px">
          ...
          <div id="productModalRich" style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px">
            ...
            <div id="productModalCaloriesRow" style="display:none"><strong style="color:#ffd86b">พลังงาน:</strong> <span id="productModalCalories" style="margin-left:6px;color:#dcd6c8"></span></div>
            <div id="productModalSizeRow" style="display:none"><strong style="color:#ffd86b">ขนาด/เสิร์ฟ:</strong> <span id="productModalSize" style="margin-left:6px;color:#dcd6c8"></span></div>
            <div id="productModalAllergensRow" style="display:none"><strong style="color:#ffd86b">สารก่อภูมิแพ้:</strong> <span id="productModalAllergens" style="margin-left:6px;color:#dcd6c8"></span></div>
            <div id="productModalPrepRow" style="display:none"><strong style="color:#ffd86b">เวลาเตรียม:</strong> <span id="productModalPrep" style="margin-left:6px;color:#dcd6c8"></span></div>
...
function showPreviewForCard(card, event) {
  ensureProductPreviewAndModal();
  const preview = document.getElementById('productPreview');
  const img = document.getElementById('productPreviewImg');
  const name = document.getElementById('productPreviewName');
  const price = document.getElementById('productPreviewPrice');
  const desc = document.getElementById('productPreviewDesc');
  const id = card.dataset.id;
  ...
  const p = card.dataset.price ? `฿${card.dataset.price}` : card.querySelector('.product-price')?.textContent || '';
  const d = card.querySelector('.product-description')?.textContent || '';
  img.src = image; name.textContent = title; price.textContent = p; desc.textContent = d;

  // position near card (right middle)
  const r = card.getBoundingClientRect();
  const left = Math.min(window.innerWidth - 300, r.right + 12);
  const top = Math.max(10, r.top + 8);
...
function openProductModal(productId){
  ...
  const image = card.dataset.image || card.querySelector('.product-img')?.src || '';
  const title = card.dataset.name || card.querySelector('.product-name')?.textContent || '';
  const price = card.dataset.price || (card.querySelector('.product-price')?.textContent.replace(/[^0-9.-]+/g,'')||0);
  ...
  setRow('productModalCaloriesRow', calories, 'productModalCalories');
  setRow('productModalSizeRow', size, 'productModalSize');
